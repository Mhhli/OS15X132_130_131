...
thread_yield (void) 
{
  ...
  old_level = intr_disable ();
  if (cur != idle_thread) 
      list_insert_ordered (&ready_list,&cur->elem,thread_cmp_priority,0);//add
...
//  thread_yield();
    ...
    {
	struct lock *l=list_entry(list_front(&thread_current()->donate_lock_list), struct lock, elem);
	temp_priority=l->holder->old_priority;
	l->older_priority=new_priority;
	l->holder->old_priority=new_priority;
